...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Check Auth with a small timeout to prevent hanging
        console.log('📊 Dashboard: Checking auth status...');
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 10000); // 10s timeout

        const authResponse = await fetch('/api/auth/verify', { signal: controller.signal });
        ...
        if (settingsData.success) {
          ...
          console.log('📊 Dashboard: Store name set to:', settingsData.settings.storeName);
        }
      } catch (error) {
